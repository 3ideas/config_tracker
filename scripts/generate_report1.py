...
# AUTOGENERATED! DO NOT EDIT! File to edit: 04_generate_report1.py.ipynb (unless otherwise specified).
...
__all__ = ['get_output_from_cmd', 'run_subprocess', 'verbose', 'show_progress', 'GitRepositoryReader',
           ...
           'remove_files_from_output', 'get_report_name_refs', 'make_output_directories',
           'generate_difference_report_page', 'BranchComparisonReport', 'FileChangeHistoryReportForBranch']
...
def run_subprocess(command):
    ...
    try:
        ...
        if retcode < 0:
            print("Command: %s, was terminated by signal: %s" % (command, retcode), file=sys.stderr)
        elif retcode > 0:
            print("Command: %s, returned: "%command, retcode, file=sys.stderr)
    except OSError as e:
...
class GitRepositoryReader:
    ...
    def get_all_filenames_in_branch(self,branch=None):
        ...
        filenames = []
        for line in lines:
            if line.startswith('database') or line.startswith('files') or line.startswith('last_updated.md'):
    ...
    def copy_file_to_output(self,filename, git_reference=None): #     git_get_file:
        ...
        if file_contents is None:
            print("File: %s does not exist in ref/branch: %s" % (filename,git_reference),file=sys.stderr)
            return
...
def copyfile_fullpath(file_name,dest_dir):
    ...
    if not os.path.isdir(dir_to_create):
        try:
            ...
            print("Directory '%s' cannot be created, skipping file: %s" % (error,file_name),file=sys.stderr)
            return
    try:
        shutil.copyfile(file_name,dest_full_name)
    except Exception as e:
        print("File cannot be copied to dest : %s, skipping file: %s" % (e,file_name),file=sys.stderr)
...
def mkdir_for_fullpath(filename_output_location):
    ...
    if not os.path.isdir(path):
        try:
            ...
            print("Directory '%s' cannot be created, skipping file: %s" % (error,path),file=sys.stderr)
            return False
    return True
...
def generate_difference_report_page(r,repository_filename,branch1, branch2,output_file_name , description,title):
    ...
    if repository_filename.endswith('.csv'):
        ...
        run_subprocess('daff diff %s %s  >diff.csv'%(file1,file2))
        diff_table_html = get_output_from_cmd('daff.py render --fragment diff.csv')
        run_subprocess('rm diff.csv')
        ...
        run_subprocess('rm %s'%output_file_name)
...
if __name__ == "__main__" and not IN_NOTEBOOK:
    ...
    ap.add_argument('-s', '--stylesheets', required=False,
                    help='location of directory holding the stylesheets', default='./styles',)
